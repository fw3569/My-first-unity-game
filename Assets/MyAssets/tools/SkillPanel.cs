...
public class SkillPanel : MonoBehaviour
{
  ...
  private List<SkillBox>item_boxs = new();
  ...
  void Awake()
  {
    ...
    for(int i=0;i<row_size;++i){
      ...
      for(int j=0;j<col_size;++j){
        ...
        x+=box_distance_x;
        SkillBox new_item_box = new_obj.GetComponent<SkillBox>();
  ...
  public void UpdateItems(){
    ...
    int id = 0;
    int box_id = 0;
    foreach(string skill_name in player.skills){
      if(id>=(page_num+1)*page_size){
        break;
      }
      if(id>=page_num*page_size){
        SkillBox skill_box = item_boxs[box_id];
        skill_box.UpdateItem(skill_name);
        ++box_id;
      }
      ++id;
    }
