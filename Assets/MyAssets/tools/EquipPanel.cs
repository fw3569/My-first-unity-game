...
public class EquipPanel : MonoBehaviour
{
  ...
  private List<EquipBox>item_boxs = new();
  ...
  void Awake()
  {
    ...
    for(int i=0;i<row_size;++i){
      ...
      for(int j=0;j<col_size;++j){
        ...
        x+=box_distance_x;
        EquipBox new_item_box = new_obj.GetComponent<EquipBox>();
  ...
  public void UpdateItems(){
    ...
    int id = 0;
    int box_id = 0;
    foreach(KeyValuePair<string,int> item in player.equips){
      if(id>=(page_num+1)*page_size){
        break;
      }
      if(id>=page_num*page_size){
        EquipBox item_box = item_boxs[box_id];
  ...
  public void PageDown(){
    print("PageDown");
    if((page_num+1)*page_size<player.equips.Count){
      ++page_num;
      UpdateItems();
    }
  }
